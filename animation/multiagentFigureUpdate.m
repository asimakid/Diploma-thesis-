function multiagentFigureUpdate(x,N_veh,cntr,L,W,enable_CG_trace,enable_rearAxle_trace,save_anim_frames,axisMode,plotAxisLimits,maxa,enable_CU_trace,enable_CD_trace,fname)
...
%Update the limits of the road 
   if (enable_CU_trace==1)
      ...
      axisTight=axis(handle.axes(1));
      sys(1)=min(X)-L/2-0.1; % new Xmin as smaller of auto (tight axis) or X_ic
 ...
 % if Config.enable_rearAxle_trace==1
   ...
   if (axisMode==0) % -> GROW-TO-FIT from initial user-supplied axis limits
      ...
      axis(handle.axes(1),[sys(1:2),-3-maxa,maxa+3]); % so make the assignment to the graphics window
