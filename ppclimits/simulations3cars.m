...
L = sqrt(L)
lamda = L+5; %radius of circle of dc must bme bigger the w/2
if lamda < L
...
j = 1:n;
x0 = zeros(4*n,1);
x0(4*(j-1)+1) = [4,7,8]*6; %init xi
x0(4*(j-1)+2) = [0,-2,+3]*1; %init yi
x0(4*(j-1)+3) = [0,0,0]*5; %init thetai
x0(4*(j-1)+4) = [32,20,20]*1; %init vi
...
for t = 1:4:length(T)
    ...
    multiagentFigureUpdate(tempx,N_veh,cntr,Lcar-1,Wcar-0.5,enable_CG_trace,...
